...
export function qqSdk() {
    $('body').delegate('[cm-sdk-qq]', 'click', function () {
        if (!this._sdk)
            this._sdk = utils.StrToJson($(this).attr('cm-sdk-qq'));

        var type = this._sdk.title.type;
        var title = $(this._sdk.title).test();
        var description = $(this._sdk.description).test();
        var thumb = $(this._sdk.thumb).attr('src');
        var webpageUrl = $(this._sdk.webpageUrl).attr('href');

        wechat(type, title, description, thumb, webpageUrl);
    })
}